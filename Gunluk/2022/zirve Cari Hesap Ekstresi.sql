...
SELECT CONVERT(NUMERIC(20,2),ISNULL(TUTARB,0)) as "Borctutar",CONVERT(NUMERIC(20,2),ISNULL(TUTARA,0)) AS "Alacaktutar",* FROM 
(
...
(
	 SELECT REF as CBKISLEMREF,TARIH,SAAT,BORA,ISLEMTURU,ISLEMAC,SUBE,ISLEMTIPI,
        ...
        WHEN ISLEMTURU = 181 THEN 380
         ...
         AND CARIP_ID IN (SELECT P_ID FROM CARIGEN WHERE ((isNull(Detay,'')<>'H') AND (isNull(GORUNSUN,'')<>'HAYIR')) and (CRK='320.1900645197'))
        ...
        select REF,TARIH,SAAT,'B',ISLEMTURU,ISLEMAC,SUBE,ISLEMTIPI,
         ...
         AND CARI2P_ID IN (SELECT P_ID FROM CARIGEN WHERE ((isNull(Detay,'')<>'H') AND (isNull(GORUNSUN,'')<>'HAYIR')) and (CRK='320.1900645197'))
         )
         OR 
		...
		UNION ALL
          ...
          AND BORDROCEKLER.ASILCARIP_ID IN (SELECT P_ID FROM CARIGEN WHERE ((isNull(Detay,'')<>'H') AND (isNull(GORUNSUN,'')<>'HAYIR')) and (CRK='320.1900645197'))
