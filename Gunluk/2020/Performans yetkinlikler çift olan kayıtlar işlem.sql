...
PerformansAnaHedefleriId = 1 and PuanlayanPersonel.Adi is not null
...
BEGIN
       set @KalanReferans=0  
       select top 1 @KalanReferans=Id,@PuanlayanOrganizasyonSemasiId=PuanlayanOrganizasyonSemasiId from VW_PerformansSablonu where OrganizasyonSemasiId = @OrganizasyonSemasiId and PuanlayanPersonelId = @PuanlayanPersonelId and PerformansAnaHedefleriId = 1 order by Agirlik Desc
   	 
	   if (@KalanReferans>0)
	   begin
	          delete from PerformansSablonu where Id in (
	          select Id from vw_PerformansSablonu where OrganizasyonSemasiId = @OrganizasyonSemasiId and PuanlayanPersonelId = @PuanlayanPersonelId and PerformansAnaHedefleriId = 1 and Id <> @KalanReferans
			  )
	   end
