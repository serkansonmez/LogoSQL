...
FROM 
(
select PerformansDonemiId,PerformansAnaHedefleriId,avg(PuanDegeri) as ortalama,UcretPersonelId,AdiSoyadi from VW_PerformansHareketlerSeviye2 WHERE PerformansAnaHedefleriId = 1
...
left join ( select PerformansDonemiId,avg(Ortalama) as FirmaOrtalama from 
					(select PerformansDonemiId, avg(PuanDegeri) as ortalama  from VW_PerformansHareketlerSeviye2 WHERE PerformansAnaHedefleriId = 1
