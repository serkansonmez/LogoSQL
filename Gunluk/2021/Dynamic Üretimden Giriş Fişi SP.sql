...
DECLARE @ENDDATE DATETIME='2021-12-07'
...
FROM GO3DB..LG_221_01_ORFICHE ORF
INNER JOIN GO3DB..LG_221_01_ORFLINE ORFL ON ORF.LOGICALREF = ORFL.ORDFICHEREF
...
IF (@VARIANTREF=0)
BEGIN
SELECT @ONHAND=CAST(CAST(SUM(ISNULL(ONHAND,0))*(@CONVFACT1/@CONVFACT2) AS NUMERIC(28,6)) AS FLOAT) FROM GO3DB..LV_221_01_STINVTOT WHERE STOCKREF=@ITEMREF AND VARIANTREF=0 AND INVENNO=@SOURCEINDEX

END
ELSE
BEGIN
  SELECT @ONHAND=CAST(CAST(SUM(ISNULL(ONHAND,0))*(@CONVFACT1/@CONVFACT2) AS NUMERIC(28,6)) AS FLOAT)  FROM GO3DB..LV_221_01_VRNTINVTOT WHERE STOCKREF=@ITEMREF AND VARIANTREF=@VARIANTREF AND INVENNO=@SOURCEINDEX
END
...
--TEMP INSERT
SELECT @CLIENTCODE = CODE,@CLIENTNAME = DEFINITION_ FROM GO3DB..LG_221_CLCARD WHERE LOGICALREF=@CLIENTREF
SELECT @BRANCHNAME = [NAME] FROM GO3DB..L_CAPIDIV WHERE FIRMNR=221 AND NR=@BRANCH
SELECT @WHOUSENAME = [NAME] FROM GO3DB..L_CAPIWHOUSE WHERE FIRMNR=221 AND NR=@SOURCEINDEX
