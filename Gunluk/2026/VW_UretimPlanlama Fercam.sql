...
/****** Object:  View [dbo].[VW_UretimPlanlama]    Script Date: 21.04.2026 11:44:33 ******/
SET ANSI_NULLS ON
GO
...
--   select * from [VW_UretimPlanlama]  where Logicalref = 3537
...
SELECT  -- DISTINCT
  ...
  --  isnull(tblKesimSehpa.SehpaAdet,0 ) as KesimSehpaAdet,
    ...
    KD.KesimAdet AS KesimPlanlanan,
...
FROM TIGER..LG_026_PRODORD PRODORD WITH(NOLOCK)
 LEFT JOIN TIGER..LG_026_PEGGING PEGGING  WITH(NOLOCK) ON PEGGING.PEGREF = PRODORD.LOGICALREF  
left JOIN TIGER.dbo.LG_026_01_ORFLINE  AS ORFLINE  WITH(NOLOCK) ON ORFLINE.LOGICALREF = PEGGING.PORDLINEREF
    left JOIN TIGER.dbo.LG_026_01_ORFICHE AS ORFICHE  WITH(NOLOCK) ON ORFLINE.ORDFICHEREF = ORFICHE.LOGICALREF  and (ORFICHE.TRCODE = 1)
