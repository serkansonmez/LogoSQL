...
/****** Object:  View [dbo].[VW_UretimPlanlama_20260506]    Script Date: 6.05.2026 16:45:46 ******/
SET ANSI_NULLS ON
GO
...
-- select * from     UretimRotaBaglanti where  RotaKodu = 'F.019.00.330'


--insert into UretimRotaBaglanti
--select 'F.019.00.351',RotaKodu,FirinRotasi,BandoSetupRotasi,UrunModeli FROM UretimRotaBaglanti where  FercamKodu like 'F.019.00.333'





--    SELECT * FROM VW_UretimPlanlama Where RotaKodu is null





--   select * from [VW_UretimPlanlama]  where LOGICALREF = 1756

 ALTER VIEW [dbo].[VW_UretimPlanlama_20260506]  AS
WITH KesimDurumu AS (
  
  select * from Tmp_KesimIsEmriToplamlari
),
...
left join (select IsEmriNo, isnull(sum(HurdaCamAdet),0) as HurdaCamAdet from HurdaCamHareket  where OperasyonTanimiId=2  group by IsEmriNo ) as tblHurda on tblHurda.IsEmriNo = OperasyonHareket.IsEmriNo collate Turkish_CI_AS  
...
left join (select IsEmriNo, isnull(sum(HurdaCamAdet),0) as HurdaCamAdet from HurdaCamHareket  where OperasyonTanimiId=4   group by IsEmriNo ) as tblHurda on tblHurda.IsEmriNo = OperasyonHareket.IsEmriNo collate Turkish_CI_AS
...
left join (select IsEmriNo, isnull(sum(HurdaCamAdet),0) as HurdaCamAdet from HurdaCamHareket with(nolock)  where OperasyonTanimiId=5  group by IsEmriNo ) as tblHurda on tblHurda.IsEmriNo = OperasyonHareket.IsEmriNo collate Turkish_CI_AS
where OperasyonHareket.OperasyonTanimiId=5 group by OperasyonHareket.IsEmriNo  ) AS  DelikTable
        LEFT JOIN (SELECT IsEmriNo, ISNULL(SUM(SehpaAdet), 0) AS SehpaAdet FROM dbo.SehpaHareket with(nolock)  WHERE OperasyonTanimiId = 5 GROUP BY IsEmriNo) AS tblDelikSehpa 
...
left join (select IsEmriNo, isnull(sum(HurdaCamAdet),0) as HurdaCamAdet from HurdaCamHareket  where OperasyonTanimiId=6  group by IsEmriNo ) as tblHurda on tblHurda.IsEmriNo = OperasyonHareket.IsEmriNo collate Turkish_CI_AS
...
left join (select IsEmriNo, isnull(sum(HurdaCamAdet),0) as HurdaCamAdet from HurdaCamHareket  where OperasyonTanimiId IN (7,75,76,77)  group by IsEmriNo ) as tblHurda on tblHurda.IsEmriNo = OperasyonHareket.IsEmriNo collate Turkish_CI_AS
where OperasyonHareket.OperasyonTanimiId IN (7,75,76,77) group by OperasyonHareket.IsEmriNo  ) AS FirinTable
        LEFT JOIN (SELECT IsEmriNo, ISNULL(SUM(SehpaAdet), 0) AS SehpaAdet FROM dbo.SehpaHareket WHERE OperasyonTanimiId IN (7,75,76,77) GROUP BY IsEmriNo) AS tblFirinSehpa 
...
left join (select IsEmriNo, isnull(sum(HurdaCamAdet),0) as HurdaCamAdet from HurdaCamHareket  where OperasyonTanimiId=9  group by IsEmriNo ) as tblHurda on tblHurda.IsEmriNo = OperasyonHareket.IsEmriNo collate Turkish_CI_AS
...
left join (select IsEmriNo, isnull(sum(HurdaCamAdet),0) as HurdaCamAdet from HurdaCamHareket  where OperasyonTanimiId=100  group by IsEmriNo ) as tblHurda on tblHurda.IsEmriNo = OperasyonHareket.IsEmriNo collate Turkish_CI_AS
...
left join (select IsEmriNo, isnull(sum(HurdaCamAdet),0) as HurdaCamAdet from HurdaCamHareket  where OperasyonTanimiId=101 group by IsEmriNo ) as tblHurda on tblHurda.IsEmriNo = OperasyonHareket.IsEmriNo collate Turkish_CI_AS
...
SELECT    DISTINCT
    ...
    ISNULL(cast(SATG.KesimAdet as float), ISNULL(PRODORD.ACTAMOUNT, 0.00)) AS [KesimAdet],
    tblDetay.RENK AS Renk,
    tblDetay.EN AS En,
    tblDetay.BOY AS Boy,
    tblDetay.KALINLIK AS Kalinlik,
    round(tblDetay.EN * tblDetay.BOY * ORFLINE.AMOUNT / 1000000,2) AS SiparisToplamAlan,
    tblDetay.BOMBE AS Firin,
    tblDetay.SERIGRAF AS Serigraf,
    0 AS ToplamCap,
    round(tblDetay.EN * tblDetay.BOY / 10000,2) AS BirimM2,
  ...
  --  isnull(tblKesimSehpa.SehpaAdet,0 ) as KesimSehpaAdet,
    ...
    --dbo.fn_RotaMevcutDurumu_20261126(  UretimRotaBaglanti.RotaKodu, isnull(KD.KesimDurumId,1),  isnull(BD.BandoDurumId,1), 1,  isnull(CD.CncDurumId,1),  isnull(DD.DelikDurumId,1),isnull(SD.SerigrafDurumId,1), isnull(fD.FirinDurumId,1), isnull(PD.PaketlemeDurumId,1)) AS MevcutOperasyonDurumu,
    --dbo.fn_RotaSonrakiDurumu_20261126( UretimRotaBaglanti.RotaKodu, isnull(KD.KesimDurumId,1),  isnull(BD.BandoDurumId,1), 1,  isnull(CD.CncDurumId,1),  isnull(DD.DelikDurumId,1),isnull(SD.SerigrafDurumId,1), isnull(fD.FirinDurumId,1), isnull(PD.PaketlemeDurumId,1)) AS SonrakiOperasyon,
 

    -- dbo.fn_RotaMevcutDurumu ( UretimRotaBaglanti.RotaKodu,KD.OptimizasyonKodu,  isnull(KD.KesimDurumId,1),  isnull(BD.BandoDurumId,1), 1,  isnull(CD.CncDurumId,1),  isnull(DD.DelikDurumId,1),isnull(SD.SerigrafDurumId,1), isnull(fD.FirinDurumId,1), isnull(PD.PaketlemeDurumId,1)) AS MevcutOperasyonDurumu,
    --dbo.fn_RotaSonrakiDurumu ( UretimRotaBaglanti.RotaKodu,KD.OptimizasyonKodu, isnull(KD.KesimDurumId,1),  isnull(BD.BandoDurumId,1), 1,  isnull(CD.CncDurumId,1),  isnull(DD.DelikDurumId,1),isnull(SD.SerigrafDurumId,1), isnull(fD.FirinDurumId,1), isnull(PD.PaketlemeDurumId,1)) AS SonrakiOperasyon,
    CASE
    WHEN ISNULL(ORFICHE.FICHENO, '') = '' THEN N'Sipariþ aþamasýnda'
    WHEN ISNULL(TRK.EksikDosyaDurumu, N'Ürün+Varyant Eksik') <> N'Tamam' THEN N'Teknik Resim bekleniyor'
    WHEN ISNULL(PRODORD.FICHENO, '') = '' THEN N'Üretim Emri oluþturulacak'
    WHEN ISNULL(SATG.KesimAdet, ISNULL(PRODORD.ACTAMOUNT, 0)) <= 0 THEN N'Sipariþ Adet Girilecek'
    WHEN ISNULL(KD.OptimizasyonKodu, '') = '' THEN N'Optimizasyon yapýlacak'
    ELSE dbo.fn_RotaMevcutDurumu(
            UretimRotaBaglanti.RotaKodu,
            KD.OptimizasyonKodu,
            ISNULL(KD.KesimDurumId,1),
            ISNULL(BD.BandoDurumId,1),
            1,
            ISNULL(CD.CncDurumId,1),
            ISNULL(DD.DelikDurumId,1),
            ISNULL(SD.SerigrafDurumId,1),
            ISNULL(FD.FirinDurumId,1),
            ISNULL(PD.PaketlemeDurumId,1)
    )
END AS MevcutOperasyonDurumu,
    CASE
    WHEN ISNULL(ORFICHE.FICHENO, '') = '' THEN N'Sipariþ aþamasýnda'
    WHEN ISNULL(TRK.EksikDosyaDurumu, N'Ürün+Varyant Eksik') <> N'Tamam' THEN N'Teknik Resim bekleniyor'
    WHEN ISNULL(PRODORD.FICHENO, '') = '' THEN N'Üretim Emri oluþturulacak'
...
END AS SonrakiOperasyon,
	...
	-- select * from TIGER..LG_026_PEGGING where PEGREF  = 4582
    ,PEGGING.VARIANTREF
    ,BandoSetupRotasi
    ,UrunModeli
    ,tblDetay.TOPLAM_DELIK_ADET as ToplamDelikAdet
    ,tblDetay.TOPLAM_CAP_SAYISI as ToplamCapSayisi
    ,CLCARD.SPECODE4 AS PaketlemeRotasi
     ,TRK.TeknikResimVarMi
    ,TRK.VaryantResimVarMi
    ,TRK.EksikDosyaDurumu
    ,KD.KesimAdet AS KesimProsesCikanAdet 
FROM TIGER.dbo.LG_026_01_ORFICHE AS ORFICHE WITH(NOLOCK)
INNER JOIN TIGER.dbo.LG_026_01_ORFLINE AS ORFLINE WITH(NOLOCK)
    ON ORFLINE.ORDFICHEREF = ORFICHE.LOGICALREF
   AND ORFICHE.TRCODE = 1
LEFT JOIN TIGER..LG_026_PEGGING PEGGING WITH(NOLOCK)
    ON PEGGING.PORDLINEREF = ORFLINE.LOGICALREF
LEFT JOIN TIGER..LG_026_PRODORD PRODORD WITH(NOLOCK)
    ON PRODORD.LOGICALREF = PEGGING.PEGREF
LEFT JOIN TIGER.dbo.SiparisAdetTerminGiris SATG WITH (NOLOCK)
    ON SATG.SiparisSatirRef = ORFLINE.LOGICALREF
    
    left JOIN TIGER.dbo.LG_026_ITEMS AS ITEMS  WITH(NOLOCK) ON ORFLINE.STOCKREF = ITEMS.LOGICALREF  and ITEMS.CODE is not null
      LEFT JOIN TIGER.dbo.TeknikResimKontrol TRK WITH (NOLOCK)        ON TRK.LOGICALREF = ITEMS.LOGICALREF
    left JOIN TIGER.dbo.LG_026_CLCARD AS CLCARD  WITH(NOLOCK) ON ORFLINE.CLIENTREF = CLCARD.LOGICALREF 
    left   JOIN (SELECT ITEMREF, CLIENTREF, ICUSTSUPCODE FROM TIGER.dbo.LG_026_SUPPASGN  WITH(NOLOCK) ) AS TEDARIKCI 
        ON CLCARD.LOGICALREF = TEDARIKCI.CLIENTREF AND ITEMS.LOGICALREF = TEDARIKCI.ITEMREF  
   
    --LEFT JOIN TIGER..LG_026_PRODORD PRODORD  WITH(NOLOCK) ON PEGGING.PEGREF = PRODORD.LOGICALREF
   -- LEFT JOIN TIGER..LG_XT050_026 tblDetay ON tblDetay.PARLOGREF = ORFLINE.STOCKREF
    LEFT JOIN TIGER..LG_XT050_026 tblDetay  WITH(NOLOCK) ON tblDetay.PARLOGREF = ITEMS.LOGICALREF
	left join UretimRotaBaglanti  WITH(NOLOCK) on (UretimRotaBaglanti.FercamKodu collate Turkish_CI_AS = ITEMS.CODE) OR (UretimRotaBaglanti.FercamKodu  collate Turkish_CI_AS LIKE ITEMS.CODE + '-__' collate Turkish_CI_AS)
 ...
 --   left join (select IsEmriNo, isnull(sum(HurdaCamAdet),0) as HurdaCamAdet from HurdaCamHareket  group by IsEmriNo ) as tblHurda on tblHurda.IsEmriNo = OperasyonHareket.IsEmriNo collate Turkish_CI_AS
...
--where OperasyonHareket.OperasyonTanimiId=1 group by OperasyonHareket.IsEmriNo,IsIstasyonlariId ,HurdaCamAdet ) AS KesimTable on KesimTable.IsEmriNo = PRODORD.FICHENO collate Turkish_CI_AS
 	WHERE  ORFLINE.LOGICALREF is not null --        (ORFICHE.TRCODE = 1)  and 
 --   ITEMS.CODE is not null
    

--and  PRODORD.FICHENO  = '2604.0024'
    
    
    -- PRODORD.FICHENO =   '2601.0126'    -- AND  KD.PlanlananAdet > 0  -- (ORFLINE.AMOUNT - ORFLINE.SHIPPEDAMOUNT > 0) and --
	--AND ORFICHE.DATE_ BETWEEN DATEADD(dd, -365, PRODORD.DATE_) AND DATEADD(dd, 365, PRODORD.DATE_)
	--  AND ITEMS.CODE  = '44 003'
	-- PRODORD.FICHENO =   '2026.0249'
